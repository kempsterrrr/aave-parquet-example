...
const processor = new evm_processor_1.EvmBatchProcessor()
    ...
    .setBlockRange({ from: 16291127 }) // Aave V3 Pool deployment block on mainnet (Dec 29, 2022)
    .addLog({
    address: [POOL],
    topic0: [
...
// ---- File store + Parquet table (TEST CONFIG) ----
...
const database = new file_store_1.Database({
    ...
    dest: new file_store_1.LocalDest('./test-data'),
    chunkSizeMb: 1 // 1MB chunks for faster testing
});
// ---- Run ----
